...
# OCN evaluation with detector-predicted boxes (YOLO).
#
# Usage:
#   bash scripts/eval_exlpose_fge_ocn_detector.sh <pose_ckpt.pth> [yolo_weights.pt] [res50|hrnet]
...
OUT_DIR="$ROOT_DIR/release/protocol_detector_eval_ocn/$TS"
TMP_DIR="$ROOT_DIR/.tmp_eval_ann"

if [[ -z "$POSE_CKPT" ]]; then
  echo "Usage: bash scripts/eval_exlpose_fge_ocn_detector.sh <pose_ckpt.pth> [yolo_weights.pt] [res50|hrnet]"
  exit 1
fi
...
for s in "${SPLITS[@]}"; do
  ...
  DET_JSON="$OUT_DIR/det_OCN_${s}.json"
  LOG="$OUT_DIR/OCN_${s}.log"
  ...
  python scripts/gen_yolo_bbox_json.py \
    ...
    --out-json "$DET_JSON" \
    --imgsz 960 --conf 0.10 --iou 0.70 --device 0 --batch 16 > "$OUT_DIR/det_OCN_${s}.build.log" 2>&1
...
echo "Saved detector-box OCN report: $OUT_DIR"
